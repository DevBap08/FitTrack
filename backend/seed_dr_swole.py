...
PROGRAM_NAME = "Dr. Swole's 4 Day BB & DB Only"

WORKOUTS = [
    {
        "name": "Upper 1 (Dr. Swole)",
        "exercises": [
...
with engine.connect() as conn:
    ...
    for workout in WORKOUTS:
        ...
        for idx, (name, sets, reps, muscle) in enumerate(workout["exercises"]):
            conn.execute(
                text("INSERT INTO exercises (plan_id, name, target_sets, target_reps, target_weight, muscle_group, order_index) VALUES (:p, :n, :s, :r, :w, :m, :o)"),
                {
                    "p": plan_id,
                    "n": name,
                    "s": sets,
